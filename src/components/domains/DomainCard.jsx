...
// The hook always shows — it is the one line teams compare domains on, so
// it must not be behind a hover. Only the explore affordance is revealed on
// hover/focus, with its space reserved so nothing shifts layout.
const DomainCard = ({ domain, className, floatActive, motionOK, onSelect, wrapperRef, panelRef }) => {
  ...
  return (
    <div data-enter className={`opacity-0 ${className ?? ""}`}>
      <div
        ref={(el) => {
          floatEl.current = el;
          wrapperRef?.(el);
      ...
      >
        <Link
          ...
          className="group relative flex min-h-[17rem] flex-col justify-between gap-8 overflow-hidden rounded-3xl border border-white/[0.08] bg-[#0b0b0b] p-7 no-underline outline-none transition-[border-color,box-shadow,transform] duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] focus-visible:border-lime/40 focus-visible:shadow-[0_0_0_1px_rgba(200,255,0,0.5)] hover:-translate-y-2 hover:border-lime/25 hover:shadow-[0_28px_80px_rgba(0,0,0,0.55)] md:min-h-[21rem] md:gap-10 md:p-9"
        >
          ...
          <div className="relative">
            ...
            <p className="mt-4 max-w-[34ch] font-general text-[0.92rem] leading-[1.7] text-white/60">
              {domain.hook}
            </p>
            <span className="pointer-events-none mt-3 inline-flex translate-y-2 items-center gap-1.5 font-mono text-[0.66rem] uppercase tracking-[0.2em] text-lime opacity-0 transition-[opacity,transform] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-focus-visible:translate-y-0 group-focus-visible:opacity-100 group-hover:translate-y-0 group-hover:opacity-100 [@media(hover:none)]:translate-y-0 [@media(hover:none)]:opacity-100">
              Explore the brief
