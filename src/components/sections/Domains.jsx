        ...
        <div className="border-t border-white/[0.06]">
          {DOMAINS.map((d, i) => (
            <Link
              ...
              to={`/domains/${d.slug}`}
              className="group grid grid-cols-[2.5rem_1fr] items-center gap-x-[clamp(1.25rem,3vw,3rem)] border-b border-white/[0.06] py-[clamp(1.5rem,3vw,2rem)] no-underline opacity-0 transition-colors duration-300 hover:bg-lime/[0.018] sm:grid-cols-[3.5rem_1fr_1.5fr]"
            >
              ...
              {/* The arrow sits in the heading so the row reads as a link at
                  every width, instead of relying on a far-right icon. */}
              <h3 className="flex items-center gap-2 font-general text-[clamp(1.05rem,2vw,1.35rem)] font-extrabold leading-[1.2] tracking-[-0.02em] text-white transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:translate-x-2">
                {d.short}
                <ArrowUpRight
                  size={17}
                  strokeWidth={2}
                  aria-hidden="true"
                  className="shrink-0 text-white/45 transition-colors duration-300 group-hover:text-lime"
                />
              </h3>

              <p className="col-start-2 pt-2 font-general text-[0.9rem] leading-[1.7] text-white/60 transition-colors duration-[0.4s] group-hover:text-white/85 sm:col-start-3 sm:row-start-1 sm:pt-0">
                {d.compare.outcome}
              </p>
            </Link>
          ))}
